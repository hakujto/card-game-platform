...
RSpec.describe "Api::Tournaments::TournamentJudges", type: :request do
  before(:each) do
    ...
    @dep_tournament = Tournament.create!({ public_id: SecureRandom.uuid, name: 'test', status: :draft, format: :standard, tournament_type: :swiss, max_players: 2, entry_fee: '0.00', prize_pool: '0.00', start_time: Time.now, end_time: nil, is_online: true, created_at: Time.now, season_id: @aux_season.id, organizer_id: @aux_player.id })
  end
