...
RSpec.describe "Api::Tournaments::Matches", type: :request do
  ...
  describe "PATCH /api/matches/:id/transitions/pending-to-active" do
    let!(:match) { Match.create!(valid_attributes).tap { |r| r.class.where(id: r.id).update_all(status: Match.statuses['pending']); r.reload } }
  ...
  describe "PATCH /api/matches/:id/transitions/active-to-completed" do
    let!(:match) { Match.create!(valid_attributes).tap { |r| r.class.where(id: r.id).update_all(status: Match.statuses['active']); r.reload } }
