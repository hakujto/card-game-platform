...
RSpec.describe "Api::Marketplace::TradeBids", type: :request do
  before(:each) do
    ...
    @dep_listing = TradeListing.create!({ status: :active, listing_type: :trade_offer, asking_price: '0.00', auction_start_price: '0.00', auction_end_time: Time.now, foil: true, condition: :mint, quantity: 1, created_at: Time.now, seller_id: @aux_player.id, card_id: @aux_card.id })
  end
