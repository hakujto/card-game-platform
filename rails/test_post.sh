...
ID_Product=$(curl -s -X POST "$BASE/products" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"product_type\": \"single_card\", \"price\": 1, \"stock\": 0, \"active\": true, \"discount_percent\": 1, \"description\": \"foo_description\", \"image_url\": \"https://example.com/foo\", \"featured\": true, \"card_id\": ${ID_Card:-null}, \"card_set_id\": ${ID_CardSet:-null}}" | extract_id)
...
ID_DraftSession=$(curl -s -X POST "$BASE/draft_sessions" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"waiting_for_players\", \"draft_type\": \"booster\", \"seats\": 2, \"time_per_pick_seconds\": 1, \"created_at\": \"2024-01-01T00:00:00Z\", \"completed_at\": null, \"card_set_id\": ${ID_CardSet:-null}}" | extract_id)
echo "DraftSession id=$ID_DraftSession"

ID_Deck=$(curl -s -X POST "$BASE/decks" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"description\": \"foo_description\", \"format\": \"standard\", \"is_public\": true, \"is_tournament_legal\": false, \"archetype\": \"aggro\", \"wins\": 0, \"losses\": 0, \"draws\": 0, \"created_at\": \"2024-01-01T00:00:00Z\", \"updated_at\": \"2024-01-01T00:00:00Z\", \"player_id\": ${ID_Player:-null}}" | extract_id)
echo "Deck id=$ID_Deck"

ID_Friendship=$(curl -s -X POST "$BASE/friendships" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"pending\", \"created_at\": \"2024-01-01T00:00:00Z\", \"requester_id\": ${ID_Player:-null}, \"receiver_id\": ${ID_Player:-null}}" | extract_id)
...
ID_Article=$(curl -s -X POST "$BASE/articles" \
  -H "Content-Type: application/json" \
  -d "{\"title\": \"foo_title\", \"slug\": \"foo_slug\", \"body\": \"foo_body\", \"excerpt\": \"foo_excerpt\", \"cover_image_url\": \"https://example.com/foo\", \"status\": \"draft\", \"article_type\": \"guide\", \"language\": \"e_n\", \"view_count\": 0, \"likes_count\": 0, \"is_featured\": true, \"published_at\": \"2024-01-01T00:00:00Z\", \"created_at\": \"2024-01-01T00:00:00Z\", \"updated_at\": \"2024-01-01T00:00:00Z\", \"author_id\": ${ID_Player:-null}, \"featured_deck_id\": ${ID_Deck:-null}}" | extract_id)
echo "Article id=$ID_Article"

ID_Stream=$(curl -s -X POST "$BASE/streams" \
  -H "Content-Type: application/json" \
  -d "{\"title\": \"foo_title\", \"stream_url\": \"https://example.com/foo\", \"status\": \"scheduled\", \"platform\": \"twitch\", \"language\": \"e_n\", \"is_official\": true, \"viewer_count_peak\": 0, \"scheduled_start\": \"2024-01-01T00:00:00Z\", \"actual_start\": null, \"ended_at\": null, \"vod_url\": \"https://example.com/foo\", \"tournament_id\": ${ID_Tournament:-null}, \"streamer_id\": ${ID_Player:-null}}" | extract_id)
echo "Stream id=$ID_Stream"

ID_PlayerAchievement=$(curl -s -X POST "$BASE/player_achievements" \
  -H "Content-Type: application/json" \
  -d "{\"earned_at\": \"2024-01-01T00:00:00Z\", \"progress\": 0, \"is_completed\": false, \"player_id\": ${ID_Player:-null}, \"achievement_id\": ${ID_Achievement:-null}}" | extract_id)
echo "PlayerAchievement id=$ID_PlayerAchievement"

ID_PlayerSeasonStats=$(curl -s -X POST "$BASE/player_season_statses" \
  -H "Content-Type: application/json" \
  -d "{\"wins\": 0, \"losses\": 0, \"draws\": 1, \"tournament_wins\": 0, \"highest_rank\": \"bronze\", \"season_points\": 0, \"player_id\": ${ID_Player:-null}, \"season_id\": ${ID_Season:-null}}" | extract_id)
...
ID_CardRuling=$(curl -s -X POST "$BASE/card_rulings" \
  -H "Content-Type: application/json" \
  -d "{\"ruling_text\": \"foo_ruling_text\", \"published_at\": \"2024-01-01\", \"source\": \"foo_source\", \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "CardRuling id=$ID_CardRuling"

ID_CardAbility=$(curl -s -X POST "$BASE/card_abilities" \
  -H "Content-Type: application/json" \
  -d "{\"ability_type\": \"keyword\", \"keyword\": \"foo_keyword\", \"ability_text\": \"foo_ability_text\", \"timing\": \"any\", \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "CardAbility id=$ID_CardAbility"

ID_PlayerCollection=$(curl -s -X POST "$BASE/player_collections" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"foil\": true, \"condition\": \"mint\", \"acquired_at\": \"2024-01-01T00:00:00Z\", \"acquired_via\": \"purchase\", \"player_id\": ${ID_Player:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "PlayerCollection id=$ID_PlayerCollection"

ID_CraftingRecipe=$(curl -s -X POST "$BASE/crafting_recipes" \
  -H "Content-Type: application/json" \
  -d "{\"dust_cost\": 1, \"is_available\": true, \"result_card_id\": ${ID_Card:-null}}" | extract_id)
echo "CraftingRecipe id=$ID_CraftingRecipe"

ID_TradeListing=$(curl -s -X POST "$BASE/trade_listings" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"active\", \"listing_type\": \"fixed_price\", \"asking_price\": \"1.00\", \"auction_start_price\": \"1.00\", \"auction_current_bid\": \"1.00\", \"auction_end_time\": \"2024-01-01T00:00:00Z\", \"foil\": true, \"condition\": \"mint\", \"quantity\": 1, \"description\": \"foo_description\", \"created_at\": \"2024-01-01T00:00:00Z\", \"expires_at\": \"2024-01-01T00:00:00Z\", \"seller_id\": ${ID_Player:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "TradeListing id=$ID_TradeListing"

ID_CardPriceHistory=$(curl -s -X POST "$BASE/card_price_histories" \
  -H "Content-Type: application/json" \
  -d "{\"price_date\": \"2024-01-01\", \"avg_price\": \"0.00\", \"min_price\": 0, \"max_price\": \"1.00\", \"volume\": 0, \"foil\": true, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "CardPriceHistory id=$ID_CardPriceHistory"

ID_DraftParticipant=$(curl -s -X POST "$BASE/draft_participants" \
  -H "Content-Type: application/json" \
  -d "{\"seat_number\": 1, \"joined_at\": \"2024-01-01T00:00:00Z\", \"session_id\": ${ID_DraftSession:-null}, \"player_id\": ${ID_Player:-null}}" | extract_id)
echo "DraftParticipant id=$ID_DraftParticipant"

ID_DeckCard=$(curl -s -X POST "$BASE/deck_cards" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"is_commander\": false, \"deck_id\": ${ID_Deck:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "DeckCard id=$ID_DeckCard"

ID_DeckSideboardCard=$(curl -s -X POST "$BASE/deck_sideboard_cards" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"deck_id\": ${ID_Deck:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "DeckSideboardCard id=$ID_DeckSideboardCard"

ID_DeckTagAssignment=$(curl -s -X POST "$BASE/deck_tag_assignments" \
  -H "Content-Type: application/json" \
  -d "{\"deck_id\": ${ID_Deck:-null}, \"tag_id\": ${ID_DeckTag:-null}}" | extract_id)
echo "DeckTagAssignment id=$ID_DeckTagAssignment"

ID_OrderItem=$(curl -s -X POST "$BASE/order_items" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"price_at_purchase\": 0, \"foil\": true, \"order_id\": ${ID_Order:-null}, \"product_id\": ${ID_Product:-null}}" | extract_id)
echo "OrderItem id=$ID_OrderItem"

ID_ArticleTagAssignment=$(curl -s -X POST "$BASE/article_tag_assignments" \
  -H "Content-Type: application/json" \
  -d "{\"article_id\": ${ID_Article:-null}, \"tag_id\": ${ID_ArticleTag:-null}}" | extract_id)
echo "ArticleTagAssignment id=$ID_ArticleTagAssignment"

ID_ArticleComment=$(curl -s -X POST "$BASE/article_comments" \
  -H "Content-Type: application/json" \
  -d "{\"body\": \"foo_body\", \"is_hidden\": true, \"created_at\": \"2024-01-01T00:00:00Z\", \"article_id\": ${ID_Article:-null}, \"author_id\": ${ID_Player:-null}, \"parent_comment_id\": ${ID_ArticleComment:-null}}" | extract_id)
echo "ArticleComment id=$ID_ArticleComment"

ID_TournamentJudge=$(curl -s -X POST "$BASE/tournament_judges" \
  -H "Content-Type: application/json" \
  -d "{\"role\": \"head_judge\", \"tournament_id\": ${ID_Tournament:-null}, \"player_id\": ${ID_Player:-null}}" | extract_id)
echo "TournamentJudge id=$ID_TournamentJudge"

ID_TournamentRegistration=$(curl -s -X POST "$BASE/tournament_registrations" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"registered\", \"seed\": null, \"final_standing\": null, \"points_earned\": 0, \"registered_at\": \"2024-01-01T00:00:00Z\", \"tournament_id\": ${ID_Tournament:-null}, \"player_id\": ${ID_Player:-null}, \"deck_id\": ${ID_Deck:-null}}" | extract_id)
echo "TournamentRegistration id=$ID_TournamentRegistration"

ID_TournamentRound=$(curl -s -X POST "$BASE/tournament_rounds" \
  -H "Content-Type: application/json" \
  -d "{\"round_number\": 1, \"status\": \"pending\", \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": null, \"time_limit_minutes\": 1, \"tournament_id\": ${ID_Tournament:-null}}" | extract_id)
echo "TournamentRound id=$ID_TournamentRound"

ID_TournamentPrize=$(curl -s -X POST "$BASE/tournament_prizes" \
  -H "Content-Type: application/json" \
  -d "{\"placement_from\": 1, \"placement_to\": 1, \"prize_type\": \"currency\", \"amount\": 0, \"description\": \"foo_description\", \"packs_count\": 1, \"season_points\": 1, \"tournament_id\": ${ID_Tournament:-null}}" | extract_id)
echo "TournamentPrize id=$ID_TournamentPrize"

ID_CraftingIngredient=$(curl -s -X POST "$BASE/crafting_ingredients" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"recipe_id\": ${ID_CraftingRecipe:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "CraftingIngredient id=$ID_CraftingIngredient"

ID_TradeBid=$(curl -s -X POST "$BASE/trade_bids" \
  -H "Content-Type: application/json" \
  -d "{\"amount\": 1, \"placed_at\": \"2024-01-01T00:00:00Z\", \"is_winning\": true, \"listing_id\": ${ID_TradeListing:-null}, \"bidder_id\": ${ID_Player:-null}}" | extract_id)
echo "TradeBid id=$ID_TradeBid"

ID_TradeTransaction=$(curl -s -X POST "$BASE/trade_transactions" \
  -H "Content-Type: application/json" \
  -d "{\"final_price\": 1, \"platform_fee\": 0, \"status\": \"pending\", \"completed_at\": \"2024-01-01T00:00:00Z\", \"listing_id\": ${ID_TradeListing:-null}, \"buyer_id\": ${ID_Player:-null}, \"seller_id\": ${ID_Player:-null}}" | extract_id)
echo "TradeTransaction id=$ID_TradeTransaction"

ID_DraftPick=$(curl -s -X POST "$BASE/draft_picks" \
  -H "Content-Type: application/json" \
  -d "{\"pick_number\": 1, \"pack_number\": 1, \"picked_at\": \"2024-01-01T00:00:00Z\", \"participant_id\": ${ID_DraftParticipant:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
echo "DraftPick id=$ID_DraftPick"

ID_Match=$(curl -s -X POST "$BASE/matches" \
  -H "Content-Type: application/json" \
  -d "{\"table_number\": 1, \"status\": \"pending\", \"player1_wins\": 0, \"player2_wins\": 0, \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": null, \"result_notes\": \"foo_result_notes\", \"round_id\": ${ID_TournamentRound:-null}, \"player1_id\": ${ID_Player:-null}, \"player2_id\": ${ID_Player:-null}}" | extract_id)
echo "Match id=$ID_Match"

ID_AwardedPrize=$(curl -s -X POST "$BASE/awarded_prizes" \
  -H "Content-Type: application/json" \
  -d "{\"final_placement\": 1, \"awarded_at\": \"2024-01-01T00:00:00Z\", \"claimed\": false, \"claimed_at\": \"2024-01-01T00:00:00Z\", \"prize_id\": ${ID_TournamentPrize:-null}, \"player_id\": ${ID_Player:-null}}" | extract_id)
echo "AwardedPrize id=$ID_AwardedPrize"

ID_TradeDispute=$(curl -s -X POST "$BASE/trade_disputes" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"open\", \"reason\": \"item_not_received\", \"description\": \"foo_description\", \"resolution\": \"foo_resolution\", \"opened_at\": \"2024-01-01T00:00:00Z\", \"resolved_at\": null, \"transaction_id\": ${ID_TradeTransaction:-null}, \"opened_by_id\": ${ID_Player:-null}, \"resolved_by_id\": ${ID_Player:-null}}" | extract_id)
