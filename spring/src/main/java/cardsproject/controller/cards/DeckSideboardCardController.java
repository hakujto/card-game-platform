...
public class DeckSideboardCardController {
    ...
    public ResponseEntity<?> show(@PathVariable Long id) {
        return service.findById(id)
            ...
            .orElse(ResponseEntity.status(404).body(java.util.Map.of("error", "DeckSideboardCard not found")));
    }
    ...
    public ResponseEntity<?> delete(@PathVariable Long id) {
        if (service.findById(id).isEmpty()) return ResponseEntity.status(404).body(java.util.Map.of("error", "DeckSideboardCard not found"));
        service.delete(id);
