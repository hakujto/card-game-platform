...
public class CouponController {
    ...
    public ResponseEntity<?> show(@PathVariable Long id) {
        return service.findById(id)
            ...
            .orElse(ResponseEntity.status(404).body(java.util.Map.of("error", "Coupon not found")));
    }

    @PutMapping("/{id}")
    public ResponseEntity<?> update(@PathVariable Long id, @Valid @RequestBody Coupon entity) {
        if (service.findById(id).isEmpty()) return ResponseEntity.status(404).body(java.util.Map.of("error", "Coupon not found"));
        entity.setId(id);
