...
public class ArticleTagAssignmentController {
    ...
    public ResponseEntity<?> show(@PathVariable Long id) {
        return service.findById(id)
            ...
            .orElse(ResponseEntity.status(404).body(java.util.Map.of("error", "ArticleTagAssignment not found")));
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<?> delete(@PathVariable Long id) {
        if (service.findById(id).isEmpty()) return ResponseEntity.status(404).body(java.util.Map.of("error", "ArticleTagAssignment not found"));
        service.delete(id);
