...
public class SeasonApiTests : IClassFixture<SeasonApiTests.TestFactory>
{
    ...
    public SeasonApiTests(TestFactory factory)
    {
    ...
    {
        var response = await _client.GetAsync("/api/seasons?q=test");
        Assert.Equal(HttpStatusCode.OK, response.StatusCode);
    }
}
