...
ID_Card=$(curl -s -X POST "$BASE/cards" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"cardType\": \"Creature\", \"rarity\": \"Common\", \"manaCost\": 1, \"manaColors\": \"White\", \"attack\": 1, \"defense\": 1, \"loyalty\": 1, \"description\": \"foo_description\", \"flavorText\": \"foo_flavor_text\", \"imageUrl\": \"https://example.com/foo\", \"artistName\": \"foo_artist_name\", \"legalFormats\": \"Standard\", \"isBanned\": true, \"isRestricted\": true, \"powerLevel\": 1, \"set\": $([ -n "$ID_CardSet" ] && echo '{"id":'"$ID_CardSet"'}' || echo 'null')}" | extract_id)
echo "Card id=$ID_Card"

ID_PlayerSeasonStats=$(curl -s -X POST "$BASE/player_season_statses" \
  -H "Content-Type: application/json" \
  -d "{\"wins\": 1, \"losses\": 1, \"draws\": 1, \"tournamentWins\": 1, \"highestRank\": \"Bronze\", \"seasonPoints\": 1, \"season\": $([ -n "$ID_Season" ] && echo '{"id":'"$ID_Season"'}' || echo 'null')}" | extract_id)
echo "PlayerSeasonStats id=$ID_PlayerSeasonStats"

ID_OrderItem=$(curl -s -X POST "$BASE/order_items" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"priceAtPurchase\": \"1.00\", \"foil\": true, \"product\": $([ -n "$ID_Product" ] && echo '{"id":'"$ID_Product"'}' || echo 'null')}" | extract_id)
...
ID_Match=$(curl -s -X POST "$BASE/matches" \
  -H "Content-Type: application/json" \
  -d "{\"tableNumber\": 1, \"status\": \"Pending\", \"player1Wins\": 1, \"player2Wins\": 1, \"startedAt\": \"2024-01-01T00:00:00Z\", \"endedAt\": \"2024-01-01T00:00:00Z\", \"resultNotes\": \"foo_result_notes\", \"player1\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null'), \"player2\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null')}" | extract_id)
...
ID_DraftParticipant=$(curl -s -X POST "$BASE/draft_participants" \
  -H "Content-Type: application/json" \
  -d "{\"seatNumber\": 1, \"joinedAt\": \"2024-01-01T00:00:00Z\", \"player\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null')}" | extract_id)
echo "DraftParticipant id=$ID_DraftParticipant"

ID_ArticleComment=$(curl -s -X POST "$BASE/article_comments" \
  -H "Content-Type: application/json" \
  -d "{\"body\": \"foo_body\", \"isHidden\": true, \"createdAt\": \"2024-01-01T00:00:00Z\", \"author\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null'), \"parentComment\": $([ -n "$ID_ArticleComment" ] && echo '{"id":'"$ID_ArticleComment"'}' || echo 'null')}" | extract_id)
...
ID_TournamentPrize=$(curl -s -X POST "$BASE/tournament_prizes" \
  -H "Content-Type: application/json" \
  -d "{\"placementFrom\": 1, \"placementTo\": 1, \"prizeType\": \"Currency\", \"amount\": \"1.00\", \"description\": \"foo_description\", \"packsCount\": 1, \"seasonPoints\": 1, \"tournament\": $([ -n "$ID_Tournament" ] && echo '{"id":'"$ID_Tournament"'}' || echo 'null')}" | extract_id)
echo "TournamentPrize id=$ID_TournamentPrize"

ID_TournamentRound=$(curl -s -X POST "$BASE/tournament_rounds" \
  -H "Content-Type: application/json" \
  -d "{\"roundNumber\": 1, \"status\": \"Pending\", \"startedAt\": \"2024-01-01T00:00:00Z\", \"endedAt\": \"2024-01-01T00:00:00Z\", \"timeLimitMinutes\": 1, \"tournament\": $([ -n "$ID_Tournament" ] && echo '{"id":'"$ID_Tournament"'}' || echo 'null'), \"matches\": $([ -n "$ID_Match" ] && echo '{"id":'"$ID_Match"'}' || echo 'null')}" | extract_id)
echo "TournamentRound id=$ID_TournamentRound"

ID_Game=$(curl -s -X POST "$BASE/games" \
  -H "Content-Type: application/json" \
  -d "{\"gameNumber\": 1, \"winnerSide\": \"Player1\", \"turnsPlayed\": 1, \"durationSeconds\": 1, \"endedBy\": \"Normal\", \"replayUrl\": \"https://example.com/foo\", \"match\": $([ -n "$ID_Match" ] && echo '{"id":'"$ID_Match"'}' || echo 'null'), \"winner\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null')}" | extract_id)
echo "Game id=$ID_Game"

ID_TradeBid=$(curl -s -X POST "$BASE/trade_bids" \
  -H "Content-Type: application/json" \
  -d "{\"amount\": \"1.00\", \"placedAt\": \"2024-01-01T00:00:00Z\", \"isWinning\": true, \"listing\": $([ -n "$ID_Tradelisting" ] && echo '{"id":'"$ID_Tradelisting"'}' || echo 'null'), \"bidder\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null')}" | extract_id)
echo "TradeBid id=$ID_TradeBid"

ID_TradeTransaction=$(curl -s -X POST "$BASE/trade_transactions" \
  -H "Content-Type: application/json" \
  -d "{\"finalPrice\": \"1.00\", \"platformFee\": \"1.00\", \"status\": \"Pending\", \"completedAt\": \"2024-01-01T00:00:00Z\", \"listing\": $([ -n "$ID_Tradelisting" ] && echo '{"id":'"$ID_Tradelisting"'}' || echo 'null'), \"buyer\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null'), \"seller\": $([ -n "$ID_Player" ] && echo '{"id":'"$ID_Player"'}' || echo 'null')}" | extract_id)
echo "TradeTransaction id=$ID_TradeTransaction"

ID_DraftSession=$(curl -s -X POST "$BASE/draft_sessions" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"WaitingForPlayers\", \"draftType\": \"Booster\", \"seats\": 1, \"createdAt\": \"2024-01-01T00:00:00Z\", \"completedAt\": \"2024-01-01T00:00:00Z\", \"cardSet\": $([ -n "$ID_CardSet" ] && echo '{"id":'"$ID_CardSet"'}' || echo 'null'), \"participants\": $([ -n "$ID_DraftParticipant" ] && echo '{"id":'"$ID_DraftParticipant"'}' || echo 'null')}" | extract_id)
