...
let test_rule_commander_is_singleton () =
  (* Rule: commander_is_singleton - this body should violate the condition and yield 422/400 *)
  let body = {json|{
    "quantity": 2,
    "is_commander": true,
    "deck_id": 1,
...
let suite_deck_card = [
  ...
  Alcotest.test_case "GET /api/deck_cards/1 returns 200 or 404" `Quick (lwt_run test_get_deck_card);
  Alcotest.test_case "PUT /api/deck_cards/1 returns 200 or 404" `Quick (lwt_run test_update_deck_card);
  Alcotest.test_case "DELETE /api/deck_cards/1 returns 204 or 404" `Quick (lwt_run test_delete_deck_card);
