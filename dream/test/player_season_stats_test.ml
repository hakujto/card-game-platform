...
let setup_player_season_stats_id = ref 0
...
let do_setup () =
  ...
  setup_player_season_stats_id := 1;
  Lwt.return_unit
...
let test_get_player_season_stats () =
  ...
  let url = Printf.sprintf "/api/player_season_statses/%d" !setup_player_season_stats_id in
  let* code = get url in
...
let suite_player_season_stats = [
  ...
  Alcotest.test_case "GET /api/player_season_statses/<id> returns 200" `Quick (lwt_run test_get_player_season_stats);
]
