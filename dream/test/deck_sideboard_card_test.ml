...
let setup_deck_sideboard_card_id = ref 0
...
let do_setup () =
  ...
  let* (_, main_id) = post_for_id "/api/deck_sideboard_cards" setup_body in
  setup_deck_sideboard_card_id := main_id;
  Lwt.return_unit
...
let test_create_deck_sideboard_card () =
  ...
  let* code = post "/api/deck_sideboard_cards" create_body in
  Alcotest.(check int) "create returns 201" 201 code;
  Lwt.return_unit

let test_get_deck_sideboard_card () =
  let url = Printf.sprintf "/api/deck_sideboard_cards/%d" !setup_deck_sideboard_card_id in
  let* code = get url in
...
let suite_deck_sideboard_card = [
  ...
  Alcotest.test_case "GET /api/deck_sideboard_cards/<id> returns 200" `Quick (lwt_run test_get_deck_sideboard_card);
  Alcotest.test_case "PATCH /api/deck_sideboard_cards/<id> returns 200" `Quick (lwt_run test_update_deck_sideboard_card);
  Alcotest.test_case "DELETE /api/deck_sideboard_cards/<id> returns 204" `Quick (lwt_run test_delete_deck_sideboard_card);
