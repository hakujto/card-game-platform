...
let test_create_coupon () =
  let* code = post "/api/coupons" valid_body in
...
let test_update_coupon () =
  let* code = put "/api/coupons/1" valid_body in
...
let test_rule_valid_until_after_valid_from () =
  (* Rule: valid_until_after_valid_from - this body should violate the condition and yield 422/400 *)
  let body = {json|{
    "code": "test",
    "discount_type": "not_Percent",
    "discount_value": 1,
...
let test_rule_discount_value_positive () =
  (* Rule: discount_value_positive - this body should violate the condition and yield 422/400 *)
  let body = {json|{
    "code": "test",
    "discount_type": "not_Percent",
    "discount_value": 0,
...
let test_rule_percent_discount_range () =
  (* Rule: percent_discount_range - this body should violate the condition and yield 422/400 *)
  let body = {json|{
    "code": "test",
...
let test_rule_uses_not_exceed_max () =
  (* Rule: uses_not_exceed_max - this body should violate the condition and yield 422/400 *)
  let body = {json|{
    "code": "test",
    "discount_type": "not_Percent",
    "discount_value": 1,
...
let suite_coupon = [
  ...
  Alcotest.test_case "GET /api/coupons/1 returns 200 or 404" `Quick (lwt_run test_get_coupon);
  Alcotest.test_case "PUT /api/coupons/1 returns 200 or 404" `Quick (lwt_run test_update_coupon);
