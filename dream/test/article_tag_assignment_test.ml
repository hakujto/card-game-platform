...
let setup_article_tag_id = ref 0
let setup_article_tag_assignment_id = ref 0
...
let do_setup () =
  ...
  setup_article_tag_id := dep_id_article_tag;
  let setup_body = Printf.sprintf "{\n    \"article_id\": %d,\n    \"tag_id\": %d\n  }" !(setup_article_id) !(setup_article_tag_id) in
  let* (_, main_id) = post_for_id "/api/article_tag_assignments" setup_body in
  setup_article_tag_assignment_id := main_id;
  Lwt.return_unit
...
let test_create_article_tag_assignment () =
  ...
  let* code = post "/api/article_tag_assignments" create_body in
  Alcotest.(check int) "create returns 201" 201 code;
  Lwt.return_unit

let test_get_article_tag_assignment () =
  let url = Printf.sprintf "/api/article_tag_assignments/%d" !setup_article_tag_assignment_id in
  let* code = get url in
...
let suite_article_tag_assignment = [
  ...
  Alcotest.test_case "GET /api/article_tag_assignments/<id> returns 200" `Quick (lwt_run test_get_article_tag_assignment);
  Alcotest.test_case "DELETE /api/article_tag_assignments/<id> returns 204" `Quick (lwt_run test_delete_article_tag_assignment);
]
