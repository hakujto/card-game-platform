...
let setup_card_ability_id = ref 0
...
let test_rule_keyword_ability_requires_keyword () =
  (* Rule: keyword_ability_requires_keyword — body violates the condition *)
  let body = {json|{
...
let suite_card_ability = [
  ...
  Alcotest.test_case "GET /api/card_abilities/<id> returns 200" `Quick (lwt_run test_get_card_ability);
  Alcotest.test_case "PUT /api/card_abilities/<id> returns 200" `Quick (lwt_run test_update_card_ability);
  Alcotest.test_case "DELETE /api/card_abilities/<id> returns 204" `Quick (lwt_run test_delete_card_ability);
