...
let setup_trade_transaction_id = ref 0
...
let test_get_trade_transaction () =
  ...
  let url = Printf.sprintf "/api/trade_transactions/%d" !setup_trade_transaction_id in
  let* code = get url in
...
let suite_trade_transaction = [
  ...
  Alcotest.test_case "GET /api/trade_transactions/<id> returns 200" `Quick (lwt_run test_get_trade_transaction);
]
