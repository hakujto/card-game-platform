...
let test_create_article_tag () =
  let* code = post "/api/article_tags" valid_body in
...
let test_update_article_tag () =
  let* code = put "/api/article_tags/1" valid_body in
...
let suite_article_tag = [
  ...
  Alcotest.test_case "GET /api/article_tags/1 returns 200 or 404" `Quick (lwt_run test_get_article_tag);
  Alcotest.test_case "PUT /api/article_tags/1 returns 200 or 404" `Quick (lwt_run test_update_article_tag);
  Alcotest.test_case "DELETE /api/article_tags/1 returns 204 or 404" `Quick (lwt_run test_delete_article_tag);
]
