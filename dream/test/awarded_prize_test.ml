...
let setup_awarded_prize_id = ref 0
...
let do_setup () =
  ...
  let dep_body_tournament_prize = Printf.sprintf "{\n    \"placement_from\": 1,\n    \"placement_to\": 1,\n    \"prize_type\": \"Currency\",\n    \"amount\": 1.0,\n    \"description\": null,\n    \"packs_count\": null,\n    \"season_points\": 1,\n    \"tournament_id\": %d\n  }" !(setup_tournament_id) in
  let* (_, dep_id_tournament_prize) = post_for_id "/api/tournament_prizes" dep_body_tournament_prize in
  setup_tournament_prize_id := dep_id_tournament_prize;
  setup_awarded_prize_id := 1;
  Lwt.return_unit
...
let test_get_awarded_prize () =
  ...
  let url = Printf.sprintf "/api/awarded_prizes/%d" !setup_awarded_prize_id in
  let* code = get url in
...
let suite_awarded_prize = [
  ...
  Alcotest.test_case "GET /api/awarded_prizes/<id> returns 200" `Quick (lwt_run test_get_awarded_prize);
]
