(* Dream handlers for ArticleTagAssignment *)
open Lwt.Syntax
...
let handler_article_tag_assignment (db : (module Caqti_lwt.CONNECTION)) req =
  ...
  | `GET, ["api"; "article_tag_assignments"] ->
    let* rows = Db.collect_list Article_tag_assignment_model.get_all_q () in
    (match rows with
     | Error e -> respond_json 500 (`String (Caqti_error.show e))
     | Ok items ->
       let json = `List (List.map (fun r ->
         let j = Article_tag_assignment_model.to_yojson (Article_tag_assignment_model.row_to_t r) in
         j) items) in
  ...
  | `POST, ["api"; "article_tag_assignments"] ->
    ...
    (try
       ...
       let* ins = Db.exec Article_tag_assignment_model.insert_q params in
       (match ins with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
        | Ok () ->
          let* last_id = Db.find Article_tag_assignment_model.last_id_q () in
          (match last_id with
           | Error e -> respond_json 500 (`String (Caqti_error.show e))
           | Ok new_id ->
             let* row = Db.find_opt Article_tag_assignment_model.get_by_id_q new_id in
             (match row with
              | Error e -> respond_json 500 (`String (Caqti_error.show e))
              | Ok (Some r) ->
                let j = Article_tag_assignment_model.to_yojson (Article_tag_assignment_model.row_to_t r) in
  ...
  (* GET /api/article_tag_assignments/:id - get one *)
  | `GET, ["api"; "article_tag_assignments"; id_str] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some id ->
       let* row = Db.find_opt Article_tag_assignment_model.get_by_id_q id in
       (match row with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
        | Ok None -> respond_json 404 (`String "Not found")
        | Ok (Some r) ->
  ...
  | `DELETE, ["api"; "article_tag_assignments"; id_str] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some id ->
       let* del = Db.exec Article_tag_assignment_model.delete_q id in
       (match del with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
