(* Dream handlers for DeckSideboardCard *)
open Lwt.Syntax
...
let validate_deck_sideboard_card (j : Yojson.Safe.t) : (unit, string list) result =
  let errors = ref [] in
  if not ((match (json_float_opt j "quantity") with Some v -> v >= 1. && v <= 4. | None -> true)) then errors := "Sideboard card quantity must be between 1 and 4 copies" :: !errors;
...
let handler_deck_sideboard_card (db : (module Caqti_lwt.CONNECTION)) req =
  ...
  | `GET, ["api"; "deck_sideboard_cards"] ->
    let* rows = Db.collect_list Deck_sideboard_card_model.get_all_q () in
    (match rows with
     | Error e -> respond_json 500 (`String (Caqti_error.show e))
     | Ok items ->
       let json = `List (List.map (fun r ->
         let j = Deck_sideboard_card_model.to_yojson (Deck_sideboard_card_model.row_to_t r) in
         j) items) in
  ...
  | `POST, ["api"; "deck_sideboard_cards"] ->
    ...
    (try
       ...
       (match validate_deck_sideboard_card j with
        | Error errs -> respond_json 422 (`Assoc [("errors", `List (List.map (fun e -> `String e) errs))])
        | Ok () ->
       let params = extract_insert_params j in
       let* ins = Db.exec Deck_sideboard_card_model.insert_q params in
       (match ins with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
        | Ok () ->
          let* last_id = Db.find Deck_sideboard_card_model.last_id_q () in
          (match last_id with
           | Error e -> respond_json 500 (`String (Caqti_error.show e))
           | Ok new_id ->
             let* row = Db.find_opt Deck_sideboard_card_model.get_by_id_q new_id in
             (match row with
              | Error e -> respond_json 500 (`String (Caqti_error.show e))
              | Ok (Some r) ->
                let j = Deck_sideboard_card_model.to_yojson (Deck_sideboard_card_model.row_to_t r) in
  ...
  (* GET /api/deck_sideboard_cards/:id - get one *)
  | `GET, ["api"; "deck_sideboard_cards"; id_str] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some id ->
       let* row = Db.find_opt Deck_sideboard_card_model.get_by_id_q id in
       (match row with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
        | Ok None -> respond_json 404 (`String "Not found")
        | Ok (Some r) ->
  ...
  (* PATCH /api/deck_sideboard_cards/:id - partial update *)
  | `PATCH, ["api"; "deck_sideboard_cards"; id_str] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some id ->
       let* body = Dream.body req in
       (try
          let j = Yojson.Safe.from_string body in
          let params = extract_insert_params j in
          let (quantity, deck_id, card_id) = params in
          let upd_params = (quantity, deck_id, card_id, id) in
          let* upd = Db.exec Deck_sideboard_card_model.update_q upd_params in
          (match upd with
           | Error e -> respond_json 500 (`String (Caqti_error.show e))
           | Ok () ->
             let* row = Db.find_opt Deck_sideboard_card_model.get_by_id_q id in
  ...
  | `DELETE, ["api"; "deck_sideboard_cards"; id_str] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some id ->
       let* del = Db.exec Deck_sideboard_card_model.delete_q id in
       (match del with
        | Error e -> respond_json 500 (`String (Caqti_error.show e))
        | Ok () -> Dream.respond ~status:`No_Content ""))

  (* PATCH /api/sideboard-cards/{id}/increment - behavior increment *)
  | `PATCH, ["api"; "deck_sideboard_cards"; id_str; "_id/increment"] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some _id ->
  ...
  (* PATCH /api/sideboard-cards/{id}/decrement - behavior decrement *)
  | `PATCH, ["api"; "deck_sideboard_cards"; id_str; "_id/decrement"] ->
    (match int_of_string_opt id_str with
     | None -> respond_json 400 (`String "Invalid id")
     | Some _id ->
