...
class DeckSideboardCardRoutesTest {

    private fun testApp(block: suspend ApplicationTestBuilder.() -> Unit) = testApplication {
        application {
            configureTestDb()
            configureSerialization()
            routing { deckSideboardCardRoutes() }
        }
        block()
    }

    @Test fun `list returns 200`() = testApp {
        val r = client.get("/api/deck_sideboard_cards")
        assertEquals(HttpStatusCode.OK, r.status)
    }

    @Test fun `create returns 201`() = testApp {
        val r = client.post("/api/deck_sideboard_cards") {
            contentType(ContentType.Application.Json)
            setBody("""{"quantity": 1, "deck_id": 1, "card_id": 1}""")
        }
        assertEquals(HttpStatusCode.Created, r.status)
    }

    @Test fun `get by id returns 200`() = testApp {
        val created = client.post("/api/deck_sideboard_cards") {
            contentType(ContentType.Application.Json)
            setBody("""{"quantity": 1, "deck_id": 1, "card_id": 1}""")
        }
    ...
    @Test fun `update returns 200`() = testApp {
        ...
        val json = Json.parseToJsonElement(created.bodyAsText()).jsonObject
        val id = json["id"]!!.jsonPrimitive.int
        val r = client.patch("/api/deck_sideboard_cards/$id") {
            contentType(ContentType.Application.Json)
            setBody("""{"quantity": 1, "deck_id": 1, "card_id": 1}""")
        }
        assertEquals(HttpStatusCode.OK, r.status)
    ...
    @Test fun `get not found returns 404`() = testApp {
        val r = client.get("/api/deck_sideboard_cards/99999")
