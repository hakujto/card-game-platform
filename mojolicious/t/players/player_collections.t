...
subtest 'PlayerCollection show returns 200 or 404' => sub {
  my $res = $t->get_ok('/api/player_collections/1' => {'X-User-Id' => 'owner-1'})->tx->res;
  ok($res->code == 200 || $res->code == 404, 'status 200 or 404');
};

subtest 'PlayerCollection update returns 200 or 404' => sub {
  my $res = $t->patch_ok('/api/player_collections/1' => {'X-User-Id' => 'owner-1'} => json => { quantity => 1 })->tx->res;
  ok($res->code == 200 || $res->code == 404, 'status 200 or 404');
};

subtest 'PlayerCollection delete returns 204 or 404' => sub {
  my $res = $t->delete_ok('/api/player_collections/1' => {'X-User-Id' => 'owner-1'})->tx->res;
