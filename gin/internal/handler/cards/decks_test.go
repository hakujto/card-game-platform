...
func setupDeckDB(t *testing.T) (*gorm.DB, *gin.Engine) {
	...
	r := gin.New()
	h := handler_app.NewDeckHandler(db)
	h.RegisterRoutes(r)
	return db, r
}

func postDeck(t *testing.T, r *gin.Engine, db *gorm.DB, body map[string]interface{}) map[string]interface{} {
...
func TestDeck_Update(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("PUT", "/api/decks/"+id, bytes.NewBuffer(b))
	req.Header.Set("Content-Type", "application/json")
	r.ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
}
