...
func postCardPriceHistory(t *testing.T, r *gin.Engine, db *gorm.DB, body map[string]interface{}) map[string]interface{} {
...
func TestCardPriceHistory_Create(t *testing.T) {
...
func TestCardPriceHistory_Get(t *testing.T) {
	db, r := setupCardPriceHistoryDB(t)
	_ = db
...
func TestCardPriceHistory_Update(t *testing.T) {
	db, r := setupCardPriceHistoryDB(t)
	_ = db
	...
	w := httptest.NewRecorder()
	req, _ := http.NewRequest("PUT", "/api/card_price_histories/"+id, bytes.NewBuffer(b))
	req.Header.Set("Content-Type", "application/json")
	r.ServeHTTP(w, req)
	assert.Equal(t, http.StatusOK, w.Code)
}

func TestCardPriceHistory_Delete(t *testing.T) {
	db, r := setupCardPriceHistoryDB(t)
	_ = db
