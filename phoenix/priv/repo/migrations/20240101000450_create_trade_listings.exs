defmodule CardsProject.Repo.Migrations.CreateTradeListings do
  ...
  def change do
    create table(:trade_listings) do
      ...
      add :card_id, references(:cards, on_delete: :restrict)
      add :bids_id, references(:trade_bids, on_delete: :delete_all), null: true

      timestamps()
    end
    create index(:trade_listings, [:seller_id])
    create index(:trade_listings, [:card_id])
    create unique_index(:trade_listings, [:public_id])
  end
end
