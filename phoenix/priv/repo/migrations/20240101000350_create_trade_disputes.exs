defmodule CardsProject.Repo.Migrations.CreateTradeDisputes do
  ...
  def change do
    create table(:trade_disputes) do
      ...
      add :transaction_id, references(:trade_transactions, on_delete: :delete_all)
      add :opened_by_id, references(:players, on_delete: :restrict)
      add :resolved_by_id, references(:players, on_delete: :nilify_all), null: true

      timestamps()
    end
    create unique_index(:trade_disputes, [:transaction_id])
