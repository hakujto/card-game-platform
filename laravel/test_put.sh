...
curl -s -X PUT "$BASE/order_items/$ID_OrderItem" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"price_at_purchase\": \"1.00\", \"foil\": true, \"order_id\": $ID_Order, \"product_id\": $ID_Product}" | python3 -m json.tool
...
curl -s -X PUT "$BASE/tournament_rounds/$ID_TournamentRound" \
  -H "Content-Type: application/json" \
  -d "{\"round_number\": 1, \"status\": \"Pending\", \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": \"2024-01-01T00:00:00Z\", \"time_limit_minutes\": 1, \"tournament_id\": $ID_Tournament}" | python3 -m json.tool
...
curl -s -X PUT "$BASE/matches/$ID_Match" \
  -H "Content-Type: application/json" \
  -d "{\"table_number\": 1, \"status\": \"Pending\", \"player1_wins\": 1, \"player2_wins\": 1, \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": \"2024-01-01T00:00:00Z\", \"result_notes\": \"foo_result_notes\", \"round_id\": $ID_TournamentRound, \"player1_id\": $ID_Player, \"player2_id\": ${ID_Player:-null}}" | python3 -m json.tool
