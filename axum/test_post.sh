...
# Seed a test user so FK to users table does not fail
sqlite3 "${DATABASE_URL#sqlite:}" "INSERT OR IGNORE INTO users(id,username,email,password_hash) VALUES(1,'testuser','test@example.com','x');" 2>/dev/null || true
...
ID_Coupon=$(curl -s -X POST "$BASE/coupons" \
  -H "Content-Type: application/json" \
  -d "{\"code\": \"foo_code_$RANDOM\", \"discount_type\": \"Percent\", \"discount_value\": 1, \"min_order_value\": 1, \"max_uses\": null, \"uses_count\": 1, \"valid_from\": \"2024-01-01T00:00:00Z\", \"valid_until\": \"2024-01-01T00:00:01Z\", \"is_active\": true}" | extract_id)
...
ID_TradeListing=$(curl -s -X POST "$BASE/trade_listings" \
  -H "Content-Type: application/json" \
  -d "{\"public_id\": \"$(uuidgen | tr '[:upper:]' '[:lower:]')\", \"status\": \"Active\", \"listing_type\": \"FixedPrice\", \"asking_price\": 1.00, \"auction_start_price\": 1, \"auction_current_bid\": 1, \"auction_end_time\": \"2024-01-01T00:00:00Z\", \"foil\": true, \"condition\": \"Mint\", \"quantity\": 1, \"description\": \"foo_description\", \"created_at\": \"2024-01-01T00:00:00Z\", \"expires_at\": \"2024-01-01T00:00:00Z\", \"seller_id\": ${ID_Player:-null}, \"card_id\": ${ID_Card:-null}}" | extract_id)
