-module(deck_sideboard_card_handler_SUITE).
-include_lib("eunit/include/eunit.hrl").

-define(BASE_URL, "http://localhost:8080/api/deck_sideboard_cards").
...
per_test() ->
    mnesia:clear_table(deck_sideboard_card).

deck_sideboard_card_handler_SUITE_test_() ->
    {setup, fun setup_suite/0, fun teardown_suite/1,
        {foreach, fun per_test/0, fun(_) -> ok end, [
        fun list_deck_sideboard_cards/0,
        fun create_deck_sideboard_card/0,
        fun get_deck_sideboard_card/0,
        fun update_deck_sideboard_card/0,
        fun delete_deck_sideboard_card/0
        ]}}.

list_deck_sideboard_cards() ->
    {ok, {{_, Code, _}, _, _}} = httpc:request(get, {?BASE_URL, []}, [], []),
    ?assertEqual(200, Code).

create_deck_sideboard_card() ->
    Body = jsone:encode(valid_params()),
    {ok, {{_, Code, _}, _, _}} = httpc:request(post,
        {?BASE_URL, [], "application/json", Body}, [], []),
    ?assertEqual(201, Code).

get_deck_sideboard_card() ->
    Body = jsone:encode(valid_params()),
