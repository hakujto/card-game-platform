-module(deck_sideboard_card_handler).
...
resource_exists(Req, State) ->
    case cowboy_req:binding(id, Req) of
        ...
        Id ->
            case deck_sideboard_card_store:find(binary_to_integer(Id)) of
...
handle_get(Req, State) ->
    case cowboy_req:binding(id, Req) of
        undefined ->
            All  = deck_sideboard_card_store:all(),
...
handle_post(Req0, State) ->
    ...
    case validate_deck_sideboard_card_rules(Params) of {error, Errs} -> reply_422(Req1, Errs, State); ok ->
    Id     = deck_sideboard_card_store:next_id(),
    Record = params_to_record(Id, Params),
    ok     = deck_sideboard_card_store:insert(Record),
...
handle_put(Req0, State) ->
    ...
    {ok, ExistingRecord} = deck_sideboard_card_store:find_record(Id),
    {ok, Body, Req1} = cowboy_req:read_body(Req0),
    Params = jsone:decode(Body, [{object_format, map}]),
    Updated = merge_record(ExistingRecord, Params),
    ok = deck_sideboard_card_store:update(Updated),
...
delete_resource(Req, State) ->
    ok = deck_sideboard_card_store:delete(maps:get(<<"id">>, State)),
    {true, Req, State}.
...
merge_record(Record, Params) ->
    #deck_sideboard_card{
        id         = Record#deck_sideboard_card.id,
        quantity   = maps:get(<<"quantity">>, Params, Record#deck_sideboard_card.quantity),
        deck_id    = maps:get(<<"deck_id">>, Params, Record#deck_sideboard_card.deck_id),
        card_id    = maps:get(<<"card_id">>, Params, Record#deck_sideboard_card.card_id),
        created_at = Record#deck_sideboard_card.created_at,
        updated_at = iso_now()
    }.
...
validate_deck_sideboard_card_rules(M) ->
    Checks = [
        fun() -> case (to_number(maps:get(<<"quantity">>, M, undefined)) >= 1 andalso to_number(maps:get(<<"quantity">>, M, undefined)) =< 4) of false -> {true, <<"Sideboard card quantity must be between 1 and 4 copies">>}; _ -> false end end
    ],
