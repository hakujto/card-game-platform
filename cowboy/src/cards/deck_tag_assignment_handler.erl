-module(deck_tag_assignment_handler).
...
resource_exists(Req, State) ->
    case cowboy_req:binding(id, Req) of
        ...
        Id ->
            case deck_tag_assignment_store:find(binary_to_integer(Id)) of
...
handle_get(Req, State) ->
    case cowboy_req:binding(id, Req) of
        undefined ->
            All  = deck_tag_assignment_store:all(),
...
handle_post(Req0, State) ->
    ...
    Id     = deck_tag_assignment_store:next_id(),
    Record = params_to_record(Id, Params),
    ok     = deck_tag_assignment_store:insert(Record),
...
delete_resource(Req, State) ->
    ok = deck_tag_assignment_store:delete(maps:get(<<"id">>, State)),
    {true, Req, State}.
