...
params_to_record(Id, Params) ->
    #order{
        ...
        total      = maps:get(<<"total">>, Params, 0),
        discount_applied = maps:get(<<"discount_applied">>, Params, 0),
        currency   = maps:get(<<"currency">>, Params, <<"USD">>),
...
handle_pay(Req0, State) ->
    {ok, Body, Req1} = cowboy_req:read_body(Req0),
    Params = jsone:decode(Body, [{object_format, map}]),
    case check_guard_pay(State) of
        false -> reply_422(Req1, [<<"Guard condition not met for pay">>], State);
        true  ->
...
check_guard_pay(_Record) ->
    %% TODO: evaluate guard for pay
    true.
...
handle_transition_paid_to_processing(Req, State) ->
    ...
    case lists:member(UserRole, [<<"admin">>, <<"staff">>]) of
        false -> {false, cowboy_req:reply(403, #{<<"content-type">> => <<"application/json">>},
            jsone:encode(#{<<"error">> => <<"Insufficient role for transition Paid -> Processing">>}), Req), State};
        true  ->
...
handle_transition_processing_to_shipped(Req, State) ->
    ...
    case lists:member(UserRole, [<<"admin">>, <<"staff">>]) of
        false -> {false, cowboy_req:reply(403, #{<<"content-type">> => <<"application/json">>},
            jsone:encode(#{<<"error">> => <<"Insufficient role for transition Processing -> Shipped">>}), Req), State};
        true  ->
...
handle_transition_shipped_to_completed(Req, State) ->
    ...
    case lists:member(UserRole, [<<"admin">>, <<"staff">>]) of
        false -> {false, cowboy_req:reply(403, #{<<"content-type">> => <<"application/json">>},
            jsone:encode(#{<<"error">> => <<"Insufficient role for transition Shipped -> Completed">>}), Req), State};
        true  ->
...
handle_transition_paid_to_cancelled(Req, State) ->
    ...
    case lists:member(UserRole, [<<"admin">>, <<"staff">>]) of
        false -> {false, cowboy_req:reply(403, #{<<"content-type">> => <<"application/json">>},
            jsone:encode(#{<<"error">> => <<"Insufficient role for transition Paid -> Cancelled">>}), Req), State};
        true  ->
    ok = order_store:assert_transition(maps:get(<<"status">>, State, undefined), <<"Cancelled">>),
    ok = order_store:update_field(maps:get(<<"id">>, State), status, <<"Cancelled">>),
    cancel_behavior(State),
...
handle_transition_completed_to_refunded(Req, State) ->
    ...
    case lists:member(UserRole, [<<"admin">>]) of
        false -> {false, cowboy_req:reply(403, #{<<"content-type">> => <<"application/json">>},
            jsone:encode(#{<<"error">> => <<"Insufficient role for transition Completed -> Refunded">>}), Req), State};
        true  ->
...
handle_transition_refunded_to_completed(Req, State) ->
    %% Transition: Refunded -> Completed
    %% @deny: transition is never allowed
    {true, cowboy_req:reply(409, #{<<"content-type">> => <<"application/json">>},
        jsone:encode(#{<<"error">> => <<"Transition Refunded -> Completed is not allowed">>}), Req), State}.

handle_transition_completed_to_cancelled(Req, State) ->
    %% Transition: Completed -> Cancelled
    %% @deny: transition is never allowed
    {true, cowboy_req:reply(409, #{<<"content-type">> => <<"application/json">>},
        jsone:encode(#{<<"error">> => <<"Transition Completed -> Cancelled is not allowed">>}), Req), State}.
