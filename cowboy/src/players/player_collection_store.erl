...
-export([all/0, find/1, find_record/1, insert/1, update/1, delete/1, next_id/0, find_by_player_id/1, find_by_card_id/1, delete_by_player_id/1]).
...
find_by_player_id(FKId) ->
    F = fun() -> mnesia:match_object(#player_collection{player_id = FKId, _ = '_'}) end,
    {atomic, Records} = mnesia:transaction(F),
    [record_to_map(R) || R <- Records].

find_by_card_id(FKId) ->
    F = fun() -> mnesia:match_object(#player_collection{card_id = FKId, _ = '_'}) end,
    {atomic, Records} = mnesia:transaction(F),
    [record_to_map(R) || R <- Records].
