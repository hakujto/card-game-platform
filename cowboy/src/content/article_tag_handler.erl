-module(article_tag_handler).
-behaviour(cowboy_rest).

-export([init/2, allowed_methods/2, content_types_provided/2, content_types_accepted/2, resource_exists/2, handle_get/2, handle_post/2, allow_missing_post/2, handle_patch/2, handle_delete/2, delete_resource/2, handle_rename/2, handle_article_count/2]).
...
resource_exists(Req, State) ->
    case cowboy_req:binding(id, Req) of
        ...
        Id ->
            case article_tag_store:find(binary_to_integer(Id)) of
...
handle_post(Req0, State) ->
    ...
    Id     = article_tag_store:next_id(),
    Record = params_to_record(Id, Params),
    ok     = article_tag_store:insert(Record),
...
handle_put(Req0, State) ->
    ...
    {ok, ExistingRecord} = article_tag_store:find_record(Id),
    {ok, Body, Req1} = cowboy_req:read_body(Req0),
    Params = jsone:decode(Body, [{object_format, map}]),
    Updated = merge_record(ExistingRecord, Params),
    ok = article_tag_store:update(Updated),
...
delete_resource(Req, State) ->
    ok = article_tag_store:delete(maps:get(<<"id">>, State)),
    {true, Req, State}.
...
merge_record(Record, Params) ->
    #article_tag{
        id         = Record#article_tag.id,
        name       = maps:get(<<"name">>, Params, Record#article_tag.name),
        slug       = maps:get(<<"slug">>, Params, Record#article_tag.slug),
        created_at = Record#article_tag.created_at,
        updated_at = iso_now()
    }.

record_to_map(#article_tag{id = Id, name = Name, slug = Slug, created_at = CreatedAt, updated_at = UpdatedAt}) ->
    #{
        <<"id">> => Id,
        <<"name">> => Name,
        <<"slug">> => Slug,
