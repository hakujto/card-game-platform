...
ID_Card=$(curl -s -X POST "$BASE/cards/" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"card_type\": \"Creature\", \"rarity\": \"Common\", \"mana_cost\": 1, \"mana_colors\": \"White\", \"attack\": 1, \"defense\": 1, \"loyalty\": null, \"description\": \"foo_description\", \"flavor_text\": \"foo_flavor_text\", \"image_url\": \"https://example.com/foo\", \"artist_name\": \"foo_artist_name\", \"legal_formats\": \"Standard\", \"is_banned\": false, \"is_restricted\": false, \"power_level\": 1, \"set\": ${ID_CardSet:-null}}" | extract_id)
echo "Card id=$ID_Card"

ID_DraftSession=$(curl -s -X POST "$BASE/draft_sessions/" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"WaitingForPlayers\", \"draft_type\": \"Booster\", \"seats\": 2, \"created_at\": \"2024-01-01T00:00:00Z\", \"completed_at\": null, \"card_set\": ${ID_CardSet:-null}}" | extract_id)
...
ID_Stream=$(curl -s -X POST "$BASE/streams/" \
  -H "Content-Type: application/json" \
  -d "{\"title\": \"foo_title\", \"stream_url\": \"https://example.com/foo\", \"platform\": \"Twitch\", \"status\": \"Scheduled\", \"viewer_count_peak\": 0, \"scheduled_start\": \"2024-01-01T00:00:00Z\", \"actual_start\": null, \"ended_at\": null, \"vod_url\": \"https://example.com/foo\", \"tournament\": ${ID_Tournament:-null}, \"streamer\": ${ID_Player:-null}}" | extract_id)
...
ID_DeckCard=$(curl -s -X POST "$BASE/deck_cards/" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"is_commander\": true, \"deck\": ${ID_Deck:-null}, \"card\": ${ID_Card:-null}}" | extract_id)
...
ID_TradeDispute=$(curl -s -X POST "$BASE/trade_disputes/" \
  -H "Content-Type: application/json" \
  -d "{\"reason\": \"ItemNotReceived\", \"description\": \"foo_description\", \"status\": \"Open\", \"resolution\": \"foo_resolution\", \"opened_at\": \"2024-01-01T00:00:00Z\", \"resolved_at\": null, \"transaction\": ${ID_TradeTransaction:-null}, \"opened_by\": ${ID_Player:-null}, \"resolved_by\": ${ID_Player:-null}}" | extract_id)
echo "TradeDispute id=$ID_TradeDispute"

ID_Game=$(curl -s -X POST "$BASE/games/" \
  -H "Content-Type: application/json" \
  -d "{\"game_number\": 1, \"winner_side\": null, \"turns_played\": null, \"duration_seconds\": null, \"ended_by\": \"Normal\", \"replay_url\": \"https://example.com/foo\", \"match\": ${ID_Match:-null}, \"winner\": ${ID_Player:-null}}" | extract_id)
