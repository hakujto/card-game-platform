...
ID_Season=$(curl -s -X POST "$BASE/seasons/" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"start_date\": \"2024-01-01\", \"end_date\": \"2024-01-02\", \"format\": \"Standard\", \"is_active\": true, \"reward_description\": \"foo_reward_description\"}" | extract_id)
...
ID_Coupon=$(curl -s -X POST "$BASE/coupons/" \
  -H "Content-Type: application/json" \
  -d "{\"code\": \"foo_code\", \"discount_type\": \"Percent\", \"discount_value\": 1, \"min_order_value\": \"1.00\", \"max_uses\": 1, \"uses_count\": 0, \"valid_from\": \"2024-01-01T00:00:00Z\", \"valid_until\": \"2024-01-01T00:00:01Z\", \"is_active\": true}" | extract_id)
...
ID_Card=$(curl -s -X POST "$BASE/cards/" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"card_type\": \"Creature\", \"rarity\": \"Common\", \"mana_cost\": 1, \"mana_colors\": \"White\", \"attack\": 1, \"defense\": 1, \"loyalty\": 1, \"description\": \"foo_description\", \"flavor_text\": \"foo_flavor_text\", \"image_url\": \"https://example.com/foo\", \"artist_name\": \"foo_artist_name\", \"legal_formats\": \"Standard\", \"is_banned\": false, \"is_restricted\": false, \"power_level\": 1, \"set\": $ID_CardSet}" | extract_id)
echo "Card id=$ID_Card"

ID_DraftSession=$(curl -s -X POST "$BASE/draft_sessions/" \
  -H "Content-Type: application/json" \
  -d "{\"status\": \"WaitingForPlayers\", \"draft_type\": \"Booster\", \"seats\": 1, \"created_at\": \"2024-01-01T00:00:00Z\", \"completed_at\": \"2024-01-01T00:00:00Z\", \"card_set\": $ID_CardSet}" | extract_id)
...
ID_PlayerSeasonStats=$(curl -s -X POST "$BASE/player_season_statses/" \
  -H "Content-Type: application/json" \
  -d "{\"wins\": 1, \"losses\": 1, \"draws\": 1, \"tournament_wins\": 1, \"highest_rank\": \"Bronze\", \"season_points\": 1, \"player\": $ID_Player, \"season\": $ID_Season}" | extract_id)
echo "PlayerSeasonStats id=$ID_PlayerSeasonStats"

ID_Tournament=$(curl -s -X POST "$BASE/tournaments/" \
  -H "Content-Type: application/json" \
  -d "{\"name\": \"foo_name\", \"description\": \"foo_description\", \"format\": \"Standard\", \"tournament_type\": \"Swiss\", \"status\": \"Draft\", \"max_players\": 2, \"entry_fee\": 0, \"prize_pool\": 0, \"start_time\": \"2024-01-01T00:00:00Z\", \"end_time\": null, \"is_online\": true, \"location\": \"foo_location\", \"rules_text\": \"foo_rules_text\", \"created_at\": \"2024-01-01T00:00:00Z\", \"season\": $ID_Season, \"organizer\": $ID_Player}" | extract_id)
...
ID_DraftParticipant=$(curl -s -X POST "$BASE/draft_participants/" \
  -H "Content-Type: application/json" \
  -d "{\"seat_number\": 1, \"joined_at\": \"2024-01-01T00:00:00Z\", \"session\": $ID_DraftSession, \"player\": $ID_Player}" | extract_id)
...
ID_OrderItem=$(curl -s -X POST "$BASE/order_items/" \
  -H "Content-Type: application/json" \
  -d "{\"quantity\": 1, \"price_at_purchase\": \"1.00\", \"foil\": true, \"order\": $ID_Order, \"product\": $ID_Product}" | extract_id)
...
ID_ArticleComment=$(curl -s -X POST "$BASE/article_comments/" \
  -H "Content-Type: application/json" \
  -d "{\"body\": \"foo_body\", \"is_hidden\": true, \"created_at\": \"2024-01-01T00:00:00Z\", \"article\": $ID_Article, \"author\": $ID_Player, \"parent_comment\": ${ID_ArticleComment:-null}}" | extract_id)
...
ID_TournamentRound=$(curl -s -X POST "$BASE/tournament_rounds/" \
  -H "Content-Type: application/json" \
  -d "{\"round_number\": 1, \"status\": \"Pending\", \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": \"2024-01-01T00:00:00Z\", \"time_limit_minutes\": 1, \"tournament\": $ID_Tournament}" | extract_id)
...
ID_Match=$(curl -s -X POST "$BASE/matches/" \
  -H "Content-Type: application/json" \
  -d "{\"table_number\": 1, \"status\": \"Pending\", \"player1_wins\": 0, \"player2_wins\": 0, \"started_at\": \"2024-01-01T00:00:00Z\", \"ended_at\": \"2024-01-01T00:00:00Z\", \"result_notes\": \"foo_result_notes\", \"round\": $ID_TournamentRound, \"player1\": $ID_Player, \"player2\": ${ID_Player:-null}}" | extract_id)
