...
import { DeckCardService } from '../../services/Cards/deck_card_service.js';

const router = Router();
const service = new DeckCardService();

function validate(data: any): void {
  if (data.quantity != null && Number(data.quantity) < 1) throw new Error('quantity: must be >= 1');
  if (data.quantity != null && Number(data.quantity) > 4) throw new Error('quantity: must be <= 4');
...
router.patch('/:id', async (req, res) => {
  ...
  try {
  validate(data);
    const existing = await prisma.deckCard.findUnique({ where: { id: Number(req.params.id) } });
...
router.delete('/:id', async (req, res) => {
  try {
    await prisma.deckCard.delete({ where: { id: Number(req.params.id) } });
    res.status(204).send();
  } catch {
