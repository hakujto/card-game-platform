# cards_project — Express REST API
...
```bash
cd cards_project
bash setup.sh   # creates .env from .env.example, installs deps, sets up DB
npm run dev
```

API: http://localhost:3000/api/

> **Note:** `.env` is git-ignored. `setup.sh` creates it automatically from `.env.example`.
> Edit `.env` to change the database path or port before running.
