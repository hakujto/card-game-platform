...
class DeckReviewApiTest extends WebTestCase
{
    ...
    {
        $this->client->request('GET', '/api/deck_reviews');
    ...
    {
        $this->client->request('POST', '/api/deck_reviews', [], [], ['CONTENT_TYPE' => 'application/json'],
            json_encode([
            'rating' => 1,
    ...
    {
        $this->client->request('GET', '/api/deck_reviews/' . $this->entityId);
        $this->assertResponseIsSuccessful();
        $this->assertResponseStatusCodeSame(200);
    }
    ...
    {
        $this->client->request('PATCH', '/api/deck_reviews/' . $this->entityId, [], [], ['CONTENT_TYPE' => 'application/json'],
            json_encode(['rating' => 1])
        );
        $this->assertResponseIsSuccessful();
        $this->assertResponseStatusCodeSame(200);
    }
    ...
    {
        $this->client->request('DELETE', '/api/deck_reviews/' . $this->entityId);
