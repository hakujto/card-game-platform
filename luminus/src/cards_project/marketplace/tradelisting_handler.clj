...
(defn- insert-tradelisting! [params]
  (let [kw-params (into {} (map (fn [[k v]] [(keyword (name k)) v]) params))
        allowed  #{:listing_type :asking_price :auction_start_price :auction_current_bid :auction_end_time :foil :condition :quantity :status :description :expires_at :seller_id :card_id :bids_id}
