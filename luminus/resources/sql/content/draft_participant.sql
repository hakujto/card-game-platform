...
SELECT id, seat_number, joined_at, session_id, player_id, created_at, updated_at FROM draft_participants

-- :name get-draft-participant-by-id :? :1
-- :doc Get a single Draft Participant by id
SELECT id, seat_number, joined_at, session_id, player_id, created_at, updated_at FROM draft_participants WHERE id = :id
