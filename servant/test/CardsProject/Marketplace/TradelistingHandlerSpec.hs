...
spec = with (return app) $ do
  ...
  describe "POST /api/trade_listings" $ do
    it "creates and returns 201" $ do
      let body = [json|{"publicId": "00000000-0000-0000-0000-000000000001", "status": "Active", "listingType": "FixedPrice", "askingPrice": 1.0, "auctionStartPrice": 1.0, "auctionCurrentBid": null, "auctionEndTime": "2024-01-01T00:00:00Z", "foil": false, "condition": "Mint", "quantity": 1, "description": null, "createdAt": "2024-01-01T00:00:00", "expiresAt": null, "sellerId": 1, "cardId": 1}|]
  ...
  describe "POST /api/trade_listings rule fixed_price_requires_asking_price" $ do
    it "rejects when fixed_price_requires_asking_price violated" $ do
      let body = [json|{"publicId": "00000000-0000-0000-0000-000000000001", "status": "Active", "listingType": "FixedPrice", "askingPrice": null, "auctionStartPrice": 0.0, "auctionCurrentBid": 0.0, "auctionEndTime": "2024-01-01T00:00:00", "foil": false, "condition": "Mint", "quantity": 0, "description": "test", "createdAt": "2024-01-01T00:00:00", "expiresAt": "2024-01-01T00:00:00", "sellerId": 1, "cardId": 1}|]
  ...
  describe "POST /api/trade_listings rule auction_requires_start_price_and_end_time" $ do
    it "rejects when auction_requires_start_price_and_end_time violated" $ do
      let body = [json|{"publicId": "00000000-0000-0000-0000-000000000001", "status": "Active", "listingType": "Auction", "askingPrice": 0.0, "auctionStartPrice": null, "auctionCurrentBid": 0.0, "auctionEndTime": "2024-01-01T00:00:00", "foil": false, "condition": "Mint", "quantity": 0, "description": "test", "createdAt": "2024-01-01T00:00:00", "expiresAt": "2024-01-01T00:00:00", "sellerId": 1, "cardId": 1}|]
      resp <- request "POST" "/api/trade_listings" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400

  describe "POST /api/trade_listings rule quantity_positive" $ do
    it "rejects when quantity_positive violated" $ do
      let body = [json|{"publicId": "00000000-0000-0000-0000-000000000001", "status": "Active", "listingType": "FixedPrice", "askingPrice": 0.0, "auctionStartPrice": 0.0, "auctionCurrentBid": 0.0, "auctionEndTime": "2024-01-01T00:00:00", "foil": false, "condition": "Mint", "quantity": 10000, "description": "test", "createdAt": "2024-01-01T00:00:00", "expiresAt": "2024-01-01T00:00:00", "sellerId": 1, "cardId": 1}|]
      resp <- request "POST" "/api/trade_listings" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400
