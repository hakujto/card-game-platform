...
spec = with (return app) $ do
  ...
  describe "POST /api/coupons" $ do
    it "creates and returns 201" $ do
      let body = [json|{"code": "test", "discountType": "Percent", "discountValue": 1, "minOrderValue": 0.0, "validFrom": "2024-01-01T00:00:00", "validUntil": "2024-01-02T00:00:00Z", "isActive": false}|]
  ...
  describe "POST /api/coupons rule valid_until_after_valid_from" $ do
    it "rejects when valid_until_after_valid_from violated" $ do
      let body = [json|{"code": "test", "discountType": "Percent", "discountValue": 0.0, "minOrderValue": 0.0, "validFrom": "2024-01-02T00:00:00Z", "validUntil": "2024-01-01T00:00:00Z", "isActive": false}|]
  ...
  describe "POST /api/coupons rule discount_value_positive" $ do
    it "rejects when discount_value_positive violated" $ do
      let body = [json|{"code": "test", "discountType": "Percent", "discountValue": 0, "minOrderValue": 0.0, "validFrom": "2024-01-01T00:00:00", "validUntil": "2024-01-01T00:00:00", "isActive": false}|]
      resp <- request "POST" "/api/coupons" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400

  describe "POST /api/coupons rule percent_discount_range" $ do
    it "rejects when percent_discount_range violated" $ do
      let body = [json|{"code": "test", "discountType": "Percent", "discountValue": 101, "minOrderValue": 0.0, "validFrom": "2024-01-01T00:00:00", "validUntil": "2024-01-01T00:00:00", "isActive": false}|]
      resp <- request "POST" "/api/coupons" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400

  describe "POST /api/coupons rule uses_not_exceed_max" $ do
    it "rejects when uses_not_exceed_max violated" $ do
      let body = [json|{"code": "test", "discountType": "Percent", "discountValue": 0.0, "minOrderValue": 0.0, "validFrom": "2024-01-01T00:00:00", "validUntil": "2024-01-01T00:00:00", "isActive": false, "maxUses": "test"}|]
      resp <- request "POST" "/api/coupons" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400
