...
spec = with (return app) $ do
  ...
  describe "GET /api/article_tags?q=test" $ do
    it "returns 200" $ do
      get "/api/article_tags?q=test" `shouldRespondWith` 200
  ...
  describe "GET /api/article_tags/1" $ do
    it "returns 200, 401, or 404" $ do
      resp <- request "GET" "/api/article_tags/1" [] ""
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 200 || s == 401 || s == 403 || s == 404

  describe "PATCH /api/article_tags/1" $ do
    it "returns 200, 401, 403, or 404" $ do
      let body = [json|{"name": "test", "slug": "test"}|]
      resp <- request "PATCH" "/api/article_tags/1" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 200 || s == 401 || s == 403 || s == 404
