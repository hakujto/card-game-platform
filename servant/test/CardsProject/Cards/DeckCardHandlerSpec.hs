...
spec = with (return app) $ do
  ...
  describe "POST /api/deck_cards" $ do
    it "creates and returns 201" $ do
      let body = [json|{"quantity": 1, "isCommander": false, "deckId": 1, "cardId": 1}|]
  ...
  describe "GET /api/deck_cards/1" $ do
    it "returns 200, 401, or 404" $ do
      resp <- request "GET" "/api/deck_cards/1" [] ""
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 200 || s == 401 || s == 403 || s == 404

  describe "PATCH /api/deck_cards/1" $ do
    it "returns 200, 401, 403, or 404" $ do
      let body = [json|{"quantity": 1, "isCommander": false, "deckId": 1, "cardId": 1}|]
      resp <- request "PATCH" "/api/deck_cards/1" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 200 || s == 401 || s == 403 || s == 404
  ...
  describe "POST /api/deck_cards rule quantity_range" $ do
    it "rejects when quantity_range violated" $ do
      let body = [json|{"quantity": 5, "isCommander": false, "deckId": 1, "cardId": 1}|]
      resp <- request "POST" "/api/deck_cards" [("Content-Type","application/json")] body
      liftIO $ statusCode (simpleStatus resp) `shouldSatisfy` \s -> s == 400

  describe "POST /api/deck_cards rule commander_is_singleton" $ do
    it "rejects when commander_is_singleton violated" $ do
